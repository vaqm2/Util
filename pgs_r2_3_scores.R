...
scores3   = read.table(args[3], header = T)
pheno_cov = read.table(args[4], header = T)
p         = as.numeric(args[5])
...
scores3 = scores3 %>% select("IID", "sBayesR_UKBB_2.8M") %>% 
    rename(SCORE3 = sBayesR_UKBB_2.8M)
...
eval_df = inner_join(eval_df, scores3, by = c("IID")) %>% unique()

k = sum(eval_df$Phenotype)/nrow(eval_df)

null_model = glm(data = eval_df, Phenotype ~ . -IID -SCORE1 - SCORE2 - SCORE3)
